...
void VoxelMetadataFactory::add_constructor(uint8_t type, ConstructorFunc ctor) {
	VOXEL_ASSERT(type >= VoxelMetadata::TYPE_CUSTOM_BEGIN);
	const unsigned int i = type - VoxelMetadata::TYPE_CUSTOM_BEGIN;
	VOXEL_ASSERT_MSG(_constructors[i] == nullptr, "Type already registered");
...
void VoxelMetadataFactory::remove_constructor(uint8_t type) {
	VOXEL_ASSERT(type >= VoxelMetadata::TYPE_CUSTOM_BEGIN);
	const unsigned int i = type - VoxelMetadata::TYPE_CUSTOM_BEGIN;
	VOXEL_ASSERT_MSG(_constructors[i] != nullptr, "Type not registered");
...
ICustomVoxelMetadata *VoxelMetadataFactory::try_construct(uint8_t type) const {
	...
	VOXEL_ASSERT_RETURN_V_MSG(ctor != nullptr, nullptr, format("Custom metadata constructor not found for type {}", type));
